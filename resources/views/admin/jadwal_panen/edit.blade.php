...
<body>
    ...
    <div id="grid4" class="p-4">
        ...
        <div class="px-10 mt-10 max-h-[80%]">
            <form action="/admin/jadwal_panen/update/{{$data->id}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 gap-4 bg-[#519259]">
                    <div class="text-center text-xl text-white font-medium p-3">
                        <h1>Ubah Data Jadwal Panen</h1>
                    </div>
                </div>
                <div class="serif grid grid-cols-4 gap-4 bg-white">
                    ...
                    <div class="pr-3 pt-2 col-span-3">
                        <input type="date" class="border w-full border-black rounded-lg p-1" name="tanggalMulai" value="{{$data->tanggalMulai}}" required>
                    </div>
                    <div class="pl-3 pt-3">
                        Jumlah Baglog<span class="text-red-600">*</span>
                    </div>
                    <div class="pr-3 pt-2 col-span-3">
                        <input type="number" class="border w-full border-black rounded-lg p-1" name="jumlahBaglog" value="{{$data->jumlahBaglog}}" required>
                    </div>
                    <div class="pl-3 pt-3">
                        Musim<span class="text-red-600">*</span>
                    </div>
                    <div class="pr-3 pt-2 pb--2 col-span-3">
                        <input type="text"  class="border w-full border-black rounded-lg p-1" name="musim" value="{{$data->musim}}" required>
                    </div>
                    <div></div>
